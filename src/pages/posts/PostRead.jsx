...
function PostRead() {
  ...
  const username = useAuthStore(state=>state.username); // 로그인 하려는 의미로 가져옴
  ...
  // 글을 읽어 와서 post 란 이름으로 캐시하라
    // pno 가 바뀌면 캐시를 갱신해라
    // 반드시 갱신 조건을 줘야함 갱신 조건 안 걸면 평생 갱신 안하고 데이터 안 바뀜
  const {data, error, loading} = useSWR(['post', pno], ()=>read(pno), {revalidateOnFocus: false});
  ...
  // 리액트의 컴포넌트 기능은 렌더링 (return JSX) + 기타(side effect, 부가 효과)
    ...
    // 그럼 어디서 실행하느냐? useEffect (왜? useEffect 가 side Effect 를 관리하는 곳이기 때문) 또는 이벤트 핸들러
      ...
      // 리액트가 useEffect, 이벤트 핸들러는 렌더링 끝난 다음에 실행함
  ...
  // navigate 혹은 useEffect, eventHandler 에서 코드에서 사용하고 Nav
  ...
  return (
    <div>
      <div className='read-title mb-3 mt-3'>{data.title}</div>
      {/* 부모에 flex 를 주면 자식들의 배치 방법을 지정할 수 있다 */}
      {/* space between : 양 끝에 하나씩 배치하고 나머지 애들은 공잭을 같은 간격으로 적당히 띄워서 배치치 */}
      <div className='mb-3' style={{display:'flex', justifyContent:'space-between'}} >
        ...
        { (isLogin && !isWriter) && <GoodButton pno={pno} goodCnt={data.goodCnt} />} {/* data.goodCnt 라고 해야함 */}
      </div>
      <div>
        {/*
        ...
        */}
        <div style={{minHeight:600, backgroundColor:'#f1f1f1', padding:'10px'}} 
